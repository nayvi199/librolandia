...
<div class="container row">
	...
	<div class="col s12 m5 offset-m1">
		<br><br>
		<h5>Editorial:</h5>
		<p><img src="{{asset($product->brand->image)}}" alt="Marca {{$product->brand->name}}" class="responsive-img" width="140px"></p>
		<h5>Descripcion:</h5>
		<p style="text-align:justify">{{$product->description or 'No hay descripcion disponible.'}}</p>
		...
		<!--PARA VER LOS COLORES SEGUN LA CATEGORIA-->
		<div class="row col-sm-4">
			<img src="{{ url('img/colores.jpg') }}" width="500px" height="200px">
			<img src="" width="px" height="px">
			<img src="" width="px" height="px">
		</div>
		<h5>Clasificacion por Color: <div style="display:inline-block;width: 25px; height: 25px;background: {{$product->color->example}};"></div>
		 </h5>
		@if (sizeof($product->specifications)>0)
			<h5>Libros Dispobibles</h5>
					...
					<select name="specification_id" class="sizes">
						...
						@foreach ($product->specifications as $specification)
							@if ($specification->inventory != null && $specification->inventory->amount != 0)
								<option data-id="{{$specification->price}}" value="{{$specification->id}}"> Tipo: {{$specification->size->name }} </option>
							@endif
						@endforeach
					</select>
				</div>
		...
		@else
		<b class="red-text">Sin libros disponibles</b>
		@endif
	</div>
</div>
<div class="container row">
